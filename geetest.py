...
def _get_php(gt, challenge, session):
    ...
    num = 0
    while num <= 5:
        resp = session.get(url, proxies=proxy, params=params)
...
def crack(gt, challenge, referer):
    ...
    open(img_path + '\\' + "fullbg.jpg", "wb").write(
        requests.get("http://static.geetest.com/" + initData["fullbg"]).content)
    open(img_path + '\\' + "bg.jpg", "wb").write(
        requests.get("http://static.geetest.com/" + initData["bg"]).content)
    ...
    response = session.get("http://api.geetest.com/ajax.php",proxies=proxy,params=params)
    try:
        ...
        if 'geetest' in response.text:
            ...
            resData = json.loads(response.text[1:-1])
            print(resData)
            if "validate" not in resData:
                print(track)
            return {
                'challenge': initData['challenge'],
